...
export class TransactionParserService {
  ...
  async parseTransaction(request: TransactionParseRequestDto): Promise<TransactionParseResponseDto> {
    try {
      ...
      const transaction = await publicClient.getTransaction({
        hash: request.transactionHash as `0x${string}`,
      });
      ...
      // Fetch transaction receipt for status and gas information
      ...
      // Try to parse with 0x parser first
      try {
        if (transaction.input && transaction.input !== '0x') {
          // Create a type-compatible client for 0x parser
          const compatibleClient = publicClient as any;
          
          // Use the 0x parser to parse swap data
          const swapData = await parseSwap({
            publicClient: compatibleClient,
            transactionHash: request.transactionHash as `0x${string}`,
          });
  ...
  private parseTransactionBasic(transaction: any, functionSelector: string): any {
    // Common function selectors for swap operations
  ...
  private isSwapTransaction(parsedData: any): boolean {
    // Check for common swap indicators in parsed data
    if (!parsedData) return false;

    // Check for common swap function names
    const swapFunctions = [
      'swap',
  ...
  private async extractSwapData(parsedData: any, chainId: number): Promise<any> {
    try {
      ...
      // For basic parsing, we can't easily extract token amounts without full ABI decoding
      // But we can provide the transaction details
      ...
      // Try to extract token information from different formats if args are available
      if (parsedData.args) {
        // Handle different swap function signatures
